...
if __name__ == '__main__':
    app.run(debug=False, threading=False)
...
def update_circle_params():
    global lock_state, circle_params, change_counter, zoom_level
        ...
        while True:
            ...
            if not success:
                ...
                if circles is not None:
                    try:
                        circles_to_draw = np.round(sorted(circles, key=lambda x: x[2], reverse=True)[0][:10, :]).astype(
                            "int")  # Sort by radius and get top 2, don't want this
                        if circle_id is not None:
                            # circles_to_draw = np.round(circles[0,circle_id]).astype("int")
                            # circles_to_draw = np.round(circles[0, 0:9]).astype("int")
...
def handle_pan():
    global pan_x, pan_y, zoom_level, old_pan_x, old_pan_y, lock_state, change_counter, pan_distance
    data = request.json
    pan_dist = int(int(data["pan_distance"]) * zoom_level)
    if not lock_state:
